...
trait Huffman extends HuffmanInterface:
  ...
  //1.  chars.groupBy(identity).map(p => (p._1, p._2.length)).toList
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
    //1. freqs.sortWith((p1, p2) => p1._2< p2._2).map(p => Leaf(p._1, p._2))
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = 
    if (singleton(trees)) trees 
      ...
      val nt = makeCodeTree(t1, t2)
      
      (nt :: tail).sortWith((t1, t2) => weight(t1) < weight((t2)))}
  
  /*trees match {
      case t1 :: t2:: tail=>
        ( makeCodeTree(t1,t2):: tail).sortWith((t1,t2)=> weight(t1)< weight(t2))
      case tr if singleton(tr) => tr
    }*/
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decodeBit(rTree: CodeTree, rBits: List[Bit]): List[Char] = {
      rTree match {
        case Leaf(c, _) => if (rBits.isEmpty) List(c) else c :: decodeBit(tree, rBits)
        case Fork(l, r, _, _) =>
          if (rBits.head == 0) decodeBit(l, rBits.tail)
          else decodeBit(r, rBits.tail)
      }
    }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def charCont(tree: CodeTree)(char: Char): List[Bit] =
      tree match {
        case _: Leaf => List()
        case Fork(l, r, _, _) =>
          if (chars(l).contains(char)) 0 :: charCont(l)(char)
          else 1 :: charCont(r)(char)
      }

    text.flatMap(c => charCont(tree)(c))
  }
